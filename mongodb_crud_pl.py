...
collection = db["partial_loss_event_collection"]
...
folder = path+"/destination_folder/Partial_loss"  # setting the destination folder


# get the list of files in the target directory whch have been comverted to parquet format completely.
def get_files():
    list_of_files = []
    for filename in os.listdir(path+'/destination_folder/Partial_loss/'):
...
def partial_loss_insert(file):
    ...
    #os.remove(path+'/destination_folder/Partial_loss/'+file)
    print("o-1")
    ...
    if not result:
        ...
        print("deleted")
        metrics_temp_table_insert(file[10:29],"Partial Loss")
        os.remove(path+'/destination_folder/Partial_loss/'+file)
